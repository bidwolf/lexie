exports.getProductList = async(page) => {
    ...
    const result = await page.evaluate(() => {
        ...
        for (let index = 0; index < listNames.length; index++) {
            let productName = listNames[index];
            let productPrice = listPrices[index];
            let hiperlinkReference = listedLinks[index];
            let productActual = new Produto(productName, hiperlinkReference, productPrice);
